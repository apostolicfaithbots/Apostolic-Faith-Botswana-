...
import LiveCountdown, { useNow } from '../components/LiveCountdown'
    ...
    <div ref={revealRef}>
      <section style={{ background:'var(--color-gray-900)', color:'#fff', padding:'8rem 0 4rem' }}>
        <div className="container">
          ...
          <p className="reveal" data-delay="0.2" style={{ fontSize:'1.05rem', opacity:0.7, maxWidth:'550px' }}>Revivals, camps, celebrations, and services throughout the year.</p>
        </div>
      </section>
  ...
  return (
    <div className="reveal" data-delay={delay} style={{
      ...
      <span style={{
        ...
        fontSize:'0.58rem', letterSpacing:'0.08em', textTransform:'uppercase', fontWeight:600,
        borderRadius:'2px', marginBottom:'0.75rem',
      }}>{event.type}</span>

      <h3 style={{
        fontFamily:'var(--font-display)', fontSize:'1.25rem', color:'var(--color-black)',
        marginBottom:'0.5rem', lineHeight:1.2, paddingRight: isLive ? '4rem' : '0',
      }}>{event.title}</h3>

      <div style={{ display:'flex', flexDirection:'column', gap:'0.25rem', fontSize:'0.82rem', color:'var(--color-gray-500)', marginBottom:'0.75rem' }}>
      ...
      {isLive && (
        <div>
          <div style={{ width:'100%', height:'4px', background:'var(--color-gray-200)', borderRadius:'2px', overflow:'hidden', marginBottom:'0.35rem' }}>
            <div style={{ width:`${pct}%`, height:'100%', background:'var(--color-red)', borderRadius:'2px', transition:'width 1s linear' }} />
          </div>
          <div style={{ fontSize:'0.72rem', color:'var(--color-gray-400)' }}>{Math.round(pct)}% complete</div>
        </div>
      )}

      {isUpcoming && (
        <LiveCountdown targetDate={start} now={now} size="sm" />
      )}

      {isPast && (
        <div style={{ fontSize:'0.72rem', color:'var(--color-gray-400)', fontWeight:500, textTransform:'uppercase', letterSpacing:'0.08em' }}>Completed</div>
      )}

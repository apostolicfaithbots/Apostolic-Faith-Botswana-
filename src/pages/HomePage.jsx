...
export default function HomePage({ events, announcements, sermons, lessons, loading }) {
  ...
  // Active campmeeting / major event for the banner
  const campNotice = useMemo(() => {
    ...
    for (const c of camps) {
      ...
      if (now >= start && now <= end) {
        ...
        return { ...c, status, pct, isLive: true, countdown: null }
      }
    }
    const upcoming = camps.filter(c => new Date(c.start_date + 'T00:00:00') > now)
    if (upcoming.length) {
      const next = upcoming[0]
      const diff = new Date(next.start_date + 'T00:00:00') - now
      const d = Math.floor(diff / 86400000)
      const h = Math.floor((diff % 86400000) / 3600000)
      const m = Math.floor((diff % 3600000) / 60000)
      const s = Math.floor((diff % 60000) / 1000)
      let cd = d > 0 ? `${d}d ${h}h ${m}m` : (h > 0 ? `${h}h ${m}m ${s}s` : `${m}m ${s}s`)
      return { ...next, status: 'Upcoming', pct: 0, isLive: false, countdown: cd }
    }
    return null
  }, [events, now])

  const upcomingEvents = useMemo(() => {
    return events.filter(e => new Date(e.end_date + 'T23:59:59') >= now).slice(0, 6)
  }, [events, now])

  const typeColors = { camp: '#C9A84C', revival: '#B8282E', service: '#4A7C59', celebration: '#6B4FA0', training: '#3A6B9F', meeting: '#636568' }
  const fmtDate = (d) => { try { return format(parseISO(d), 'dd MMM') } catch { return d } }

  return (
    <div ref={revealRef}>
      {/* HERO */}
      <section ref={heroRef} style={{
        position: 'relative', height: '100vh', minHeight: '650px',
        display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: '#111',
      }}>
        <div style={{
          position: 'absolute', inset: 0,
          backgroundImage: `url(${basePath}images/church-side.jpg)`,
          backgroundSize: 'cover', backgroundPosition: 'center', filter: 'brightness(0.3)', transform: 'scale(1.05)',
        }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', color: '#fff', maxWidth: '800px', padding: '0 2rem' }}>
          <div className="hero-tag" style={{ fontSize: '0.7rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--color-gold-light)', fontWeight: 500, marginBottom: '1.5rem', opacity: 0 }}>
            Apostolic Faith Tabernacle &mdash; Gaborone, Botswana
          </div>
          <h1 className="hero-title" style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2.5rem,7vw,5rem)', fontWeight: 400, lineHeight: 1.05, marginBottom: '1.5rem', opacity: 0 }}>
            Jesus,<br /><span style={{ color: 'var(--color-gold)' }}>The Light</span> of The World
          </h1>
          <p className="hero-subtitle" style={{ fontSize: 'clamp(0.95rem,1.5vw,1.15rem)', lineHeight: 1.7, maxWidth: '550px', margin: '0 auto 2.5rem', opacity: 0, color: 'rgba(255,255,255,0.8)' }}>
      ...
      {/* CAMP MEETING NOTICE BANNER */}
      {campNotice && (
        <section style={{
          background: campNotice.isLive ? 'var(--color-red)' : 'var(--color-gray-900)',
          ...
          <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexWrap: 'wrap', gap: '1.5rem' }}>
            ...
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.15rem', fontWeight: 500 }}>
            ...
            {campNotice.countdown && (
              <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--color-gold-light)', letterSpacing: '0.04em' }}>
                {campNotice.countdown}
              </div>
            )}
          </div>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(2rem,5vw,5rem)', alignItems: 'center' }} className="two-col">
            <div>
              ...
              <h2 className="reveal" data-delay="0.1" style={{ fontSize: 'clamp(2rem,4vw,3rem)', color: 'var(--color-black)', marginBottom: '1.5rem', lineHeight: 1.1 }}>
                A Place of <span style={{ color: 'var(--color-red)' }}>Faith</span>,<br />Hope & Community
              </h2>
              <p className="reveal" data-delay="0.2" style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--color-gray-600)', marginBottom: '1rem' }}>
                The Apostolic Faith Church in Botswana is part of the worldwide Apostolic Faith Mission headquartered in Portland, Oregon, USA. We are committed to the original Pentecostal doctrines of salvation, sanctification, and the baptism of the Holy Spirit.
              </p>
              <p className="reveal" data-delay="0.25" style={{ fontSize: '1rem', lineHeight: 1.8, color: 'var(--color-gray-600)', marginBottom: '2rem' }}>
                Our services are a time to worship God through music, be encouraged by testimonies, and hear practical, Bible-based sermons.
              </p>
              <Link to="/about" className="reveal" data-delay="0.3" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600, color: 'var(--color-red)', borderBottom: '2px solid var(--color-red)', paddingBottom: '0.25rem' }}>
                About Our Church <span>&rarr;</span>
              </Link>
            </div>
            <div className="reveal" data-delay="0.2" style={{ position: 'relative' }}>
              <img src={`${basePath}images/church-front.jpg`} alt="Apostolic Faith Tabernacle"
      ...
      <section className="section" style={{ background: 'var(--color-white)' }}>
        <div className="container">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <Link to="/events" className="reveal" data-delay="0.2" style={{ fontSize: '0.8rem', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 600, color: 'var(--color-red)', borderBottom: '2px solid var(--color-red)', paddingBottom: '0.25rem' }}>
              Full Calendar &rarr;
            </Link>
          </div>
              ...
              {upcomingEvents.map((ev, i) => {
                ...
                return (
                  <div key={ev.id} className="reveal" data-delay={Math.min(i * 0.08, 0.4)} style={{ position: 'relative', display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
                    ...
                    {/* Date chip */}
                    <div style={{
                      minWidth: '70px', padding: '0.35rem 0.6rem', background: 'var(--color-gray-100)',
                      borderRadius: '4px', textAlign: 'center', flexShrink: 0,
                    }}>
                      <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', fontWeight: 600, color: 'var(--color-black)', lineHeight: 1 }}>
                        {format(parseISO(ev.start_date), 'dd')}
                      </div>
                      <div style={{ fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'var(--color-gray-500)' }}>
                    ...
                    <div style={{ flex: 1, paddingBottom: '0.5rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.25rem' }}>
                        ...
                        {isLive && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', padding: '0.1rem 0.45rem', background: 'var(--color-red)', color: '#fff', fontSize: '0.55rem', fontWeight: 700, borderRadius: '2px', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
                          <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: '#fff', animation: 'dotPulse 1.5s infinite' }} /> Live
                        </span>}
                      </div>
                      <div style={{ fontSize: '0.82rem', color: 'var(--color-gray-500)' }}>
                        {fmtDate(ev.start_date)}{ev.start_date !== ev.end_date ? ` - ${fmtDate(ev.end_date)}` : ''} &middot; {ev.location}
                        {ev.responsibility && <span> &middot; {ev.responsibility}</span>}
                      </div>
                    </div>
                  </div>
                )
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(2rem,4vw,4rem)' }} className="two-col">
            ...
            <div>
              ...
              {lessons.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {lessons.slice(0, 3).map((l, i) => (
                    <div key={l.id} className="reveal" data-delay={0.15 + i * 0.08} style={{
                      background: '#fff', padding: '1.25rem', borderRadius: '4px', border: '1px solid var(--color-gray-200)',
                    }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.3rem' }}>
                        <span style={{ padding: '0.1rem 0.4rem', background: 'var(--color-gray-100)', fontSize: '0.6rem', fontWeight: 600, textTransform: 'uppercase', borderRadius: '2px', color: 'var(--color-gray-600)', letterSpacing: '0.06em' }}>
                          {l.category.replace('_', ' ')}
                        </span>
                      </div>
                      <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', color: 'var(--color-black)', marginBottom: '0.2rem' }}>{l.title}</h3>
              ...
              <Link to="/lessons" className="reveal" data-delay="0.3" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', marginTop: '1.25rem', fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, color: 'var(--color-red)' }}>
                All Lessons &rarr;
              </Link>
            </div>
            ...
            <div>
              ...
              {sermons.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {sermons.slice(0, 3).map((s, i) => (
                    <div key={s.id} className="reveal" data-delay={0.15 + i * 0.08} style={{
                      background: '#fff', padding: '1.25rem', borderRadius: '4px', border: '1px solid var(--color-gray-200)',
                    }}>
                      ...
                      <div style={{ fontSize: '0.8rem', color: 'var(--color-gray-500)' }}>
                        {s.speaker && <span>{s.speaker} &middot; </span>}
                        {s.date && format(parseISO(s.date), 'dd MMM yyyy')}
                      </div>
                      {s.scripture_ref && <div style={{ fontSize: '0.78rem', color: 'var(--color-gray-400)', fontStyle: 'italic', marginTop: '0.2rem' }}>{s.scripture_ref}</div>}
                    </div>
                  ))}
                </div>
              ) : (
              ...
              <Link to="/sermons" className="reveal" data-delay="0.3" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', marginTop: '1.25rem', fontSize: '0.78rem', letterSpacing: '0.1em', textTransform: 'uppercase', fontWeight: 600, color: 'var(--color-red)' }}>
                All Sermons &rarr;
              </Link>
            </div>
          </div>
      ...
      <section style={{ position: 'relative', padding: 'clamp(5rem,10vw,8rem) 0', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 2, textAlign: 'center', color: '#fff' }}>
          ...
          <p className="reveal" data-delay="0.2" style={{ fontSize: '1.05rem', lineHeight: 1.8, maxWidth: '600px', margin: '0 auto 2.5rem', opacity: 0.85 }}>
            Whether visiting for the first time or looking for a church home, we invite you to worship with us in person or online every Sunday.
          </p>
